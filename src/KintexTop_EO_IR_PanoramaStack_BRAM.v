...
module KintexTop_EO_IR_Combined_HD_SDI(
    ...
    // This avoids having separate single/stack buffering paths with different
    // behavior.
    wire        ir_hd_de;
    wire        ir_hd_hsync;
    wire        ir_hd_vsync;
...
module IR640x512_GrayFrameBuffer #(
    ...
    assign wr_sample_now  = wr_vsync && wr_hsync && (wr_addr < FRAME_PIXELS) && !wr_fifo_full;
...
module IR6Modes_To_HD1080p_Buffered(
    ...
    localparam integer STACK_H       = 1024;
    ...
    IR640x512_GrayFrameBuffer u_cam0_fb (
    ...
    IR640x512_GrayFrameBuffer u_cam1_fb (
    ...
    IR640x512_GrayFrameBuffer u_cam2_fb (
    ...
    IR640x512_GrayFrameBuffer u_cam3_fb (
    ...
    IR640x512_GrayFrameBuffer u_cam4_fb (
    ...
    IR640x512_GrayFrameBuffer u_cam5_fb (
...
module IR640x512_To_HD1080p_Buffered(
    input  wire        rst_n,
    input  wire        wr_clk,
    input  wire        wr_hsync,
    input  wire        wr_vsync,
    ...
    localparam integer SRC_H         = 512;
    localparam integer FRAME_PIXELS  = SRC_W * SRC_H;
    localparam integer FRAME_ADDR_W  = 19;   // ceil(log2(327680))
    localparam integer FRAME_BITS    = FRAME_PIXELS * 8;
    localparam integer READ_LATENCY  = 2;
    ...
    // Capture selected IR frame sequentially during the source active region.
    always @(posedge wr_clk) begin
        if (!rst_n) begin
            wr_addr           <= {FRAME_ADDR_W{1'b0}};
            wr_bank           <= 1'b0;
            ...
            if (!wr_vsync && wr_vsync_d) begin
                if (wr_addr != {FRAME_ADDR_W{1'b0}}) begin
