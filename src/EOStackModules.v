module EO1920x1080_Decimate3_FrameBuffer #(
    ...
    localparam integer FRAME_BITS   = FRAME_PIXELS * 20;
    localparam integer FIFO_WIDTH   = 1 + FRAME_ADDR_W + 20;
    ...
    assign wr_sample_now   = wr_frame_active && wr_hsync &&
                             (wr_y_mod == 2'd0) && (wr_x_mod == 2'd0) &&
                             (wr_addr < FRAME_PIXELS) && !wr_fifo_full;
    assign wr_fifo_din     = {(wr_addr == (FRAME_PIXELS - 1)), wr_addr, wr_pixel};
    ...
    wire [19:0] rd_pixel_buf;
    assign rd_pixel = rd_pixel_buf;
    ...
    generate
        if (USE_ASYNC_FIFO) begin : gen_async_wr
            ...
            wire [19:0]           rd_fifo_pixel;
            wire                  wr_fifo_full_i;

            assign rd_fifo_pop     = !rd_fifo_empty;
            assign rd_fifo_last    = rd_fifo_dout[FIFO_WIDTH-1];
            assign rd_fifo_addr    = rd_fifo_dout[FRAME_ADDR_W+19:20];
            assign rd_fifo_pixel   = rd_fifo_dout[19:0];
...
module EO6Stack_To_HD1080p_Buffered(
    ...
    localparam integer STACK_H       = 720;
